    ...
    if media_fatturato > 0 or punto_pareggio_medio > 0:
        ...
        fig_gauge = go.Figure(go.Indicator(
            ...
            gauge = {
                ...
                'bar': {'color': "#1E293B", 'thickness': 0.22}, # Arco Scuro (Fatturato Reale)
                'steps': [
                ...
                'threshold': {
                    'line': {'color': "#1E3A8A", 'width': 4}, # Linea Blu Istituzionale (Soglia BEP)
                    'thickness': 0.75, 
        ...
        st.plotly_chart(fig_gauge, use_container_width=True, key="gauge_v80_institutional")
        
        # LEGENDA DINAMICA ISTITUZIONALE
        ...
        with leg_col2:
            st.markdown(f"""
                <div style='text-align:center; background-color:#EFF6FF; padding:6px; border-radius:6px; border:1px solid #BFDBFE;'>
                    <p style='font-size:11px; color:#1E3A8A; margin:0;'>🟦 <b>Linea Blu (Soglia BEP)</b></p>
                    <p style='font-size:13px; color:#1E3A8A; font-weight:700; margin:2px 0;'>€ {punto_pareggio_medio:,.2f}</p>
                </div>
            """, unsafe_allow_html=True)
